...
<div className=' flex border border-[#D5D5D5]  cursor-pointer items-center rounded-lg h-8 justify-evenly bg-[#F5F6FA] w-96'>
 <p><IoSearchSharp size={24} color='#D5D5D5' style={{}} /></p> 
<input type="text" placeholder='Search' className='focus:outline-none bg-[#F5F6FA] w-80 text-black pl-2  ' />
</div>
...
<div  className='mr-8 gap-2 flex justify-center items-center' >
 <div onClick={()=>gotoNotificationPage()} className="relative p-[8px] cursor-pointer"><IoNotifications size={25} color="orange"  />
 <span className="absolute top-0 right-0 w-4 h-4 flex justify-center items-center rounded-full bg-red-600 text-white p-1 text-[8px]">{count}</span>
 </div>
<img onClick={()=>handleEdit()} src={ profileImageUrl ? profileImageUrl:"https://st3.depositphotos.com/15648834/17930/v/450/depositphotos_179308454-stock-illustration-unknown-person-silhouette-glasses-profile.jpg"} className='w-10 h-10  rounded-full ' alt="" />
<p onClick={()=>handleEdit()} className="text-lg text-black">Bharat Preneurs</p>
</div>
</div>
  )
}

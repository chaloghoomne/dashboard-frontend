...
const AboutUs = () => {
	...
	const removeSection = (index) => {
		const updatedSectionData = formData.sections.splice(index, 0);
		setFormData({
			...formData,
	...
	const removePoint = (index, pointIndex) => {
		const updatedPoints = formData.sections[index].point.filter(
			(_, pi) => pi !== pointIndex
		);
		const updatedSections = formData.sections.map((section, i) =>
	...
	const removeSummary = (index, summaryIndex) => {
		const updatedSummary = formData.sections[index].summary.filter(
			(_, pi) => pi !== summaryIndex
		);
		const updatedSections = formData.sections.map((section, i) =>
	...
	const handleSubmit = (e) => {
		...
		axios
			.post(`${BASE_URL}/about`, formDataToSend)
					...
					{formData?.sections?.map((section, index) => (
						...
						>
							<div className="flex justify-between">
								<label className="block text-lg text-blue-600">
							...
							{section?.point?.map((point, pointIndex) => (
								...
								>
									<label className=" w-full flex flex-col text-blue-600">
										Point {pointIndex + 1}
										<textarea
											value={point}
											onChange={(e) =>
							...
							{section?.summary?.map((summary, summaryIndex) => (
								...
								>
									...
									<button
										type="button"
										onClick={() =>
											removeSummary(index, summaryIndex)
										}
										className="ml-4 bg-red-500 hover:bg-red-600 text-xs text-white py-1 px-2 rounded"
									>
										Remove

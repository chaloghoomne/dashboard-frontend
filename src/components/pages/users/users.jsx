...
import { FaEye, FaMailBulk, FaTrash } from "react-icons/fa";
...
const Users = ({ data }) => {
	...
	const [transactionsModalIsOpen, setTransactionsModalIsOpen] =
		useState(false);
  ...
  const [emailIdentities, setEmailIdentities] = useState([])
  ...
  useEffect(()=>{
    const fetchadmin = async ()=>{
     try {
            ...
            //  console.log(response.data)
             if (response){
               // console.log(response.data)
               setEmailIdentities(response.data.emailIdentities);
	...
	// 	{ name: "Visa Agent", fromEmail: "visaagent@yourdomain.com" },
	// 	{ name: "Outlook", fromEmail: "agent@outlook.com" },
	// ];

	const sendEmail = async (payload) => {
    try {
      const response = await axios.post(`${BASE_URL}admin-send-email`, payload, {
							...
							<tr key={index}>
								...
								<td className="px-6 py-1 items-center pl-12 justify-center whitespace-nowrap">
									<FaEye
										className="cursor-pointer"
										onClick={() =>
											openTransactionsModal(user?._id)
										}
									/>
								</td>
								<td className="px-6 py-1 whitespace-nowrap">
								...
								<td className="px-6 py-1 whitespace-nowrap">
									{truncateText(
										user?.passportExpiry?.slice(0, 10)
									)}
								</td>
								<td className="px-6 py-1 whitespace-nowrap">

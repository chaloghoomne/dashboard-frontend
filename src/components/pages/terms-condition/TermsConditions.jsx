...
const TermsConditions = () => {
  const [formData, setFormData] = useState({
    ...
    pageType: 'term', // Added type field
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${BASE_URL}page-by-type/terms`);
    ...
    <div className="bg-slate-300 min-h-[89%] overflow-auto p-8 text-black">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <TextEditor value = {formData.description} onChange = {handleEditorChange} required/>
        </div>
